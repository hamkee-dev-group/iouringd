...
{
    static const char message[] = "file-read";
    ...
    char file_template[] = "/tmp/iouringd-file-read-XXXXXX";
    ...
    if (set_cloexec(file_fd) != 0 ||
        write_full(file_fd, message, sizeof(message) - 1u) != 0 ||
    ...
    snprintf(socket_path,
             sizeof(socket_path),
             "/tmp/iouringd-file-read-roundtrip-%ld.sock",
             (long)getpid());
    unlink(socket_path);
