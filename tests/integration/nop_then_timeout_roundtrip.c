...
int main(int argc, char **argv)
{
    ...
    snprintf(socket_path,
             sizeof(socket_path),
             "/tmp/iouringd-nop-then-timeout-%ld.sock",
             (long)getpid());
    unlink(socket_path);
    ...
    if ((handshake.capabilities.op_mask & IOURINGD_CAPABILITY_OP_NOP) == 0u ||
        (handshake.capabilities.op_mask & IOURINGD_CAPABILITY_OP_TIMEOUT) == 0u) {
        close(fd);
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
    ...
    if (iouringd_submit_timeout(fd, UINT64_C(1000000), &second_task) != 0) {
        close(fd);
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
    ...
    if (second_task.task_id != first_task.task_id + 1u ||
