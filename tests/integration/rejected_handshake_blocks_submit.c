...
int main(int argc, char **argv)
{
    ...
    snprintf(socket_path, sizeof(socket_path), "/tmp/iouringd-reject-%ld.sock", (long)getpid());
    unlink(socket_path);
    ...
    if (write_full(fd, &request, sizeof(request)) != 0 ||
        read_full(fd, &handshake, sizeof(handshake)) != 0) {
        close(fd);
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
    ...
    if (handshake.response.status != IOURINGD_HANDSHAKE_STATUS_REJECT) {
        close(fd);
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
