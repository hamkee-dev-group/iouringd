...
static int exercise_session(int fd, uint64_t *task_id)
{
...
int main(int argc, char **argv)
{
    ...
    snprintf(socket_path, sizeof(socket_path), "/tmp/iouringd-reconnect-%ld.sock", (long)getpid());
    unlink(socket_path);
    ...
    if (exercise_session(fd, &first_task_id) != 0) {
        close(fd);
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
    ...
    if (exercise_session(fd, &second_task_id) != 0) {
        close(fd);
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
    ...
    if (second_task_id != first_task_id + 1u) {
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
        return 1;
