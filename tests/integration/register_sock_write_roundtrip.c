...
{
    static const char message[] = "ping-through-daemon";
    ...
    snprintf(socket_path,
             sizeof(socket_path),
             "/tmp/iouringd-register-write-%ld.sock",
             (long)getpid());
    unlink(socket_path);
    ...
    if (read_full(io_fds[1], peer_buf, sizeof(message) - 1u) != 0 ||
    ...
    if (iouringd_release_resource(control_fd, resource.resource_id, &released) != 0 ||
        released.resource_id != resource.resource_id) {
        close(control_fd);
        kill(child, SIGTERM);
        waitpid(child, &status, 0);
        unlink(socket_path);
