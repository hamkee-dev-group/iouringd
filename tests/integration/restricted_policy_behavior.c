...
int main(int argc, char **argv)
{
    ...
    snprintf(socket_path,
             sizeof(socket_path),
             "/tmp/iouringd-restricted-policy-%ld.sock",
             (long)getpid());
    unlink(socket_path);
    ...
    if (child == 0) {
        ...
        close(stderr_pipe[1]);
        setenv("IOURINGD_TEST_FAULT", "setup_eperm", 1);
    ...
    if (strstr(stderr_buf, "io_uring setup blocked by kernel policy or sandbox") ==
            NULL ||
        strstr(stderr_buf, "Operation not permitted") == NULL) {
        return 1;
    }
