...
TARGET = $(BIN_DIR)/iouringd
ASAN_TARGET = $(BIN_DIR)/iouringd.asan
UBSAN_TARGET = $(BIN_DIR)/iouringd.ubsan
TSAN_TARGET = $(BIN_DIR)/iouringd.tsan
TEST_TARGET = $(TEST_DIR)/test_version
ABI_CONTRACT_TARGET = $(TEST_DIR)/handshake_contract
COMPLETION_RES_CONTRACT_TARGET = $(TEST_DIR)/completion_res_contract
SUBMIT_RESULT_CONTRACT_TARGET = $(TEST_DIR)/submit_result_contract
TIMEOUT_REQUEST_CONTRACT_TARGET = $(TEST_DIR)/timeout_request_contract
CANCEL_REQUEST_CONTRACT_TARGET = $(TEST_DIR)/cancel_request_contract
PROTOCOL_TASK_KINDS_TARGET = $(TEST_DIR)/test_protocol_task_kinds
CLIENT_HANDSHAKE_VALIDATION_TARGET = $(TEST_DIR)/client_handshake_validation
CLIENT_COMPLETION_VALIDATION_TARGET = $(TEST_DIR)/client_completion_validation
CLIENT_SUBMIT_VALIDATION_TARGET = $(TEST_DIR)/client_submit_validation
TEST_DAEMON_TARGET = $(TEST_DIR)/iouringd_test_daemon
ROUNDTRIP_DAEMON_TARGET = $(TARGET)
HANDSHAKE_ROUNDTRIP_TARGET = $(TEST_DIR)/handshake_roundtrip
NOP_ROUNDTRIP_TARGET = $(TEST_DIR)/nop_roundtrip
NOP_TWO_REQUESTS_ROUNDTRIP_TARGET = $(TEST_DIR)/nop_two_requests_roundtrip
RECONNECT_TASK_ID_ROUNDTRIP_TARGET = $(TEST_DIR)/reconnect_task_id_roundtrip
GRACEFUL_SHUTDOWN_TARGET = $(TEST_DIR)/graceful_shutdown
REGISTER_SOCK_WRITE_ROUNDTRIP_TARGET = $(TEST_DIR)/register_sock_write_roundtrip
REGISTER_SOCK_READ_ROUNDTRIP_TARGET = $(TEST_DIR)/register_sock_read_roundtrip
REGISTER_FILE_READ_ROUNDTRIP_TARGET = $(TEST_DIR)/register_file_read_roundtrip
REGISTER_FILE_WRITE_ROUNDTRIP_TARGET = $(TEST_DIR)/register_file_write_roundtrip
OPENAT_FILE_READ_ROUNDTRIP_TARGET = $(TEST_DIR)/openat_file_read_roundtrip
OPENAT_CLOSE_ROUNDTRIP_TARGET = $(TEST_DIR)/openat_close_roundtrip
REGISTER_CONNECT_ROUNDTRIP_TARGET = $(TEST_DIR)/register_connect_roundtrip
REGISTER_ACCEPT_ROUNDTRIP_TARGET = $(TEST_DIR)/register_accept_roundtrip
SUBMIT_ACCEPT_EINVAL_REJECTED_TARGET = $(TEST_DIR)/submit_accept_einval_rejected
REGISTER_POLL_ROUNDTRIP_TARGET = $(TEST_DIR)/register_poll_roundtrip
REGISTER_BUFFER_FIXED_IO_ROUNDTRIP_TARGET = $(TEST_DIR)/register_buffer_fixed_io_roundtrip
CONFIG_STATS_ROUNDTRIP_TARGET = $(TEST_DIR)/config_stats_roundtrip
TRACE_ROUNDTRIP_TARGET = $(TEST_DIR)/trace_roundtrip
RELEASE_BUSY_RESOURCE_REJECTED_TARGET = $(TEST_DIR)/release_busy_resource_rejected
TWO_CLIENTS_REGISTER_SOCK_WRITE_ROUNDTRIP_TARGET = $(TEST_DIR)/two_clients_register_sock_write_roundtrip
TWO_CLIENTS_TIMEOUT_ROUNDTRIP_TARGET = $(TEST_DIR)/two_clients_timeout_roundtrip
PER_CLIENT_CREDITS_ROUNDTRIP_TARGET = $(TEST_DIR)/per_client_credits_roundtrip
PRIORITY_DISPATCH_ROUNDTRIP_TARGET = $(TEST_DIR)/priority_dispatch_roundtrip
BOUNDED_ADMISSION_ROUNDTRIP_TARGET = $(TEST_DIR)/bounded_admission_roundtrip
TIMEOUT_ROUNDTRIP_TARGET = $(TEST_DIR)/timeout_roundtrip
TIMEOUT_TWO_REQUESTS_ROUNDTRIP_TARGET = $(TEST_DIR)/timeout_two_requests_roundtrip
NOP_THEN_TIMEOUT_ROUNDTRIP_TARGET = $(TEST_DIR)/nop_then_timeout_roundtrip
TIMEOUT_THEN_NOP_ROUNDTRIP_TARGET = $(TEST_DIR)/timeout_then_nop_roundtrip
MALFORMED_TIMEOUT_REQUEST_REJECTED_TARGET = $(TEST_DIR)/malformed_timeout_request_rejected
MALFORMED_TIMEOUT_THEN_NOP_ROUNDTRIP_TARGET = $(TEST_DIR)/malformed_timeout_then_nop_roundtrip
CANCEL_REQUEST_REJECTED_TARGET = $(TEST_DIR)/cancel_request_rejected
CANCEL_SUBMISSION_REJECTED_TARGET = $(TEST_DIR)/cancel_submission_rejected
CANCEL_TIMEOUT_ROUNDTRIP_TARGET = $(TEST_DIR)/cancel_timeout_roundtrip
SUBMIT_BAD_MAGIC_REJECTED_TARGET = $(TEST_DIR)/submit_bad_magic_rejected
SUBMIT_RESERVED_BITS_REJECTED_TARGET = $(TEST_DIR)/submit_reserved_bits_rejected
CANCEL_PARTIAL_TARGET_REJECTED_TARGET = $(TEST_DIR)/cancel_partial_target_rejected
UNSUPPORTED_SUBMIT_KIND_REJECTED_TARGET = $(TEST_DIR)/unsupported_submit_kind_rejected
UNSUPPORTED_KERNEL_BEHAVIOR_TARGET = $(TEST_DIR)/unsupported_kernel_behavior
RESTRICTED_POLICY_BEHAVIOR_TARGET = $(TEST_DIR)/restricted_policy_behavior
TRACE_STDERR_ROUNDTRIP_TARGET = $(TEST_DIR)/trace_stderr_roundtrip
WAIT_COMPLETION_REJECTS_INVALID_TARGET = $(TEST_DIR)/wait_completion_rejects_invalid
COMPLETION_INVALID_TASK_ID_TARGET = $(TEST_DIR)/completion_invalid_task_id
INVALID_COMPLETION_RESPONSE_TARGET = $(TEST_DIR)/invalid_completion_response
HANDSHAKE_CLIENT_REJECTS_INVALID_RESPONSE_TARGET = $(TEST_DIR)/handshake_client_rejects_invalid_response
HANDSHAKE_INVALID_RESPONSE_TARGET = $(TEST_DIR)/handshake_invalid_response
REJECTED_HANDSHAKE_BLOCKS_SUBMIT_TARGET = $(TEST_DIR)/rejected_handshake_blocks_submit
SUBMIT_VERSION_MAJOR_REJECTED_TARGET = $(TEST_DIR)/submit_version_major_rejected
SUBMIT_RESERVED1_REJECTED_TARGET = $(TEST_DIR)/submit_reserved1_rejected
DAEMON_SRC = src/daemon/main.c src/daemon/handshake.c src/daemon/submit.c
...
ALL_BUILD_TARGETS = $(TARGET) $(ASAN_TARGET) $(UBSAN_TARGET) $(TSAN_TARGET) \
	...
	$(CLIENT_SUBMIT_VALIDATION_TARGET) $(TEST_DAEMON_TARGET) \
	$(ROUNDTRIP_DAEMON_TARGET) \
	...
	$(SUBMIT_ACCEPT_EINVAL_REJECTED_TARGET) \
	...
	$(UNSUPPORTED_KERNEL_BEHAVIOR_TARGET) \
	$(RESTRICTED_POLICY_BEHAVIOR_TARGET) \
	$(TRACE_STDERR_ROUNDTRIP_TARGET) \
...
$(TARGET): $(DAEMON_SRC)
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) -Isrc/daemon $(CFLAGS) $(filter %.c,$^) -o $@

$(ASAN_TARGET): $(DAEMON_SRC)
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) -Isrc/daemon $(CFLAGS) $(filter %.c,$^) -o $@

$(UBSAN_TARGET): $(DAEMON_SRC)
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) -Isrc/daemon $(CFLAGS) $(filter %.c,$^) -o $@

$(TSAN_TARGET): $(DAEMON_SRC)
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) -Isrc/daemon $(CFLAGS) $(filter %.c,$^) -o $@
...
$(TEST_DAEMON_TARGET): $(TARGET)
	@mkdir -p $(dir $@)
	printf '%s\n' '#!/bin/sh' 'exec ./build/bin/iouringd --registered-fds 2 "$$@"' > $@
	chmod +x $@
...
$(SUBMIT_ACCEPT_EINVAL_REJECTED_TARGET): tests/integration/submit_accept_einval_rejected.c src/lib/client.c src/lib/submit.c
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) $(DEFS) -O2 -g $(filter %.c,$^) -o $@
...
$(UNSUPPORTED_KERNEL_BEHAVIOR_TARGET): tests/integration/unsupported_kernel_behavior.c
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) $(DEFS) -O2 -g $< -o $@

$(RESTRICTED_POLICY_BEHAVIOR_TARGET): tests/integration/restricted_policy_behavior.c
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) $(DEFS) -O2 -g $< -o $@

$(TRACE_STDERR_ROUNDTRIP_TARGET): tests/integration/trace_stderr_roundtrip.c src/lib/client.c src/lib/submit.c
	@mkdir -p $(dir $@)
	$(CC) $(CPPFLAGS) $(CSTD) $(WARN) $(DEFS) -O2 -g $(filter %.c,$^) -o $@
